...
const statusOf = (r: Row): 'Open' | 'Closed' => {
  if (isLegacy(r)) { const s = String(r.status ?? 'Open'); return s === 'Closed' || s === 'Resolved' ? 'Closed' : 'Open' }
...
function nextNumber(rows: Row[], kind: Kind, disc: string): string {
  ...
  let max = 0
  for (const r of rows) { const m = String(r.rfi_number ?? '').match(re); if (m) max = Math.max(max, parseInt(m[1], 10)) }
...
export default function RFITab({ projectId, projectName, onToast }: Props) {
  ...
  const withMeta = useMemo(() => rows.map((r) => ({
    ...
    _disc: String(r.discipline ?? ''), _subject: String(r.subject ?? '')
  })), [rows])
  ...
  const agg = useMemo(() => {
    let total = 0, ans = 0
    for (const r of rows) { const pts = pointsOf(r); total += pts.length; ans += pts.filter(answered).length }
    return { total, ans }
  }, [rows])

  const startNew = (): void => setDraft({ kind: 'RFI', discipline: '', submitted_date: today(), points: [{ id: pid(), text: '', response: '' }] })
  const startEdit = (r: Row): void => {
    if (isLegacy(r)) { setLegacy(r); return }
    setDraft({ id: r.id as number, kind: (String(r.kind ?? 'RFI') as Kind), discipline: String(r.discipline ?? ''), submitted_date: String(r.submitted_date ?? today()), rfi_number: String(r.rfi_number ?? ''), points: pointsOf(r).map((p) => ({ ...p })) })
  }

  const setPoint = (i: number, patch: Partial<Point>): void =>
    setDraft((d) => (d ? { ...d, points: d.points.map((p, idx) => (idx === i ? { ...p, ...patch } : p)) } : d))
  const addPoint = (): void => setDraft((d) => (d ? { ...d, points: [...d.points, { id: pid(), text: '', response: '' }] } : d))
  const removePoint = (i: number): void => setDraft((d) => (d ? { ...d, points: d.points.filter((_, idx) => idx !== i) } : d))
  ...
  const save = async (): Promise<void> => {
    if (!draft) return
    if (!draft.discipline) { onToast('Pick a discipline', 'error'); return }
    const points = draft.points.filter((p) => String(p.text ?? '').trim() || p.image || String(p.response ?? '').trim())
    if (points.length === 0) { onToast('Add at least one point', 'error'); return }
    // Persist a derived top-level status too, so dashboards/reminders that count
    // open RFIs (x.status === 'Open') keep working with the new multi-point shape.
    const status = points.every(answered) ? 'Closed' : 'Open'
    setSaving(true)
    try {
      if (draft.id) {
        await window.api.items.update('rfi', { id: draft.id, project_id: projectId, kind: draft.kind, discipline: draft.discipline, submitted_date: draft.submitted_date, rfi_number: draft.rfi_number, points, status })
  ...
  // Naming: a single RFI → "<No>_<Project>"; multiple → "<Project>_RFI-Query".
  const exportFmt = async (fmt: 'excel' | 'word' | 'ppt'): Promise<void> => {
    ...
    if (!data.length) { onToast('No data to export', 'error'); return }
    const fileName = data.length === 1 ? `${String(data[0].rfi_number ?? 'RFI')}_${projectName}` : `${projectName}_RFI-Query`
    if (fmt === 'word') {
    ...
    // Pass the RAW rows (with their points + base64 images) so the exporter can embed images.
  ...
  // One-click export of a single RFI/Query → file named "<No>_<Project>".
  ...
  // ── Editor ──────────────────────────────────────────────────────────────────
  if (draft) {
    const numberPreview = draft.rfi_number || (draft.discipline ? nextNumber(rows, draft.kind, draft.discipline) : `${draft.kind === 'Query' ? 'QRY' : 'RFI'}-…`)
    return (
      <div className="tab-content">
        <div className="tab-toolbar">
          <div className="tab-toolbar-left">
            <button className="btn btn-secondary btn-sm" onClick={() => setDraft(null)}><Icon name="arrowLeft" size={14} /> Back to list</button>
          </div>
          <div className="tab-toolbar-right">
            <button className="btn btn-primary btn-sm" disabled={saving} onClick={save}>{saving ? 'Saving…' : draft.id ? 'Save changes' : 'Create'}</button>
          </div>
        </div>
          ...
          <div className="rfi-head">
            <label><span>Type</span>
              <select value={draft.kind} onChange={(e) => setDraft({ ...draft, kind: e.target.value as Kind })}>
                <option value="RFI">RFI</option><option value="Query">Query</option>
              </select>
            </label>
            <label><span>Discipline</span>
            ...
            <label><span>Date</span>
              <input type="date" value={draft.submitted_date} onChange={(e) => setDraft({ ...draft, submitted_date: e.target.value })} />
            </label>
                  ...
                  <div className="rfi-point-img">
                    ...
                    {p.image && (
                      <div className="rfi-thumb">
                        <img src={p.image} alt="" />
                        <button className="btn btn-secondary btn-xs" type="button" onClick={() => setPoint(i, { image: undefined })}>Remove</button>
                      </div>
                    )}
                  </div>
